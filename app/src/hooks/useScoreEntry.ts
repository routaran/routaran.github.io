...
export function useScoreEntry({
  ...
  // Initialize state
  const [team1Score, setTeam1ScoreState] = useState<number>(match.team1_score || 0);
  const [team2Score, setTeam2ScoreState] = useState<number>(match.team2_score || 0);
  ...
  const hasChanges = team1Score !== (match.team1_score || 0) || team2Score !== (match.team2_score || 0);
  ...
  }, [team1Score, team2Score, match.team1_score, match.team2_score, validationConfig, hasChanges]);
  ...
  const setScores = useCallback((team1: number, team2: number) => {
    setTeam1Score(team1);
    setTeam2Score(team2);
  ...
  // Apply common score
  const applyCommonScore = useCallback((scoreSet: { team1: number; team2: number }) => {
...
export function useScoreKeyboard(
  actions: Pick<ScoreEntryActions, 'setTeam1Score' | 'setTeam2Score' | 'submitScore' | 'resetScore'>,
  team1Score: number,
  team2Score: number,
