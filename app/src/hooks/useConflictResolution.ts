...
export interface ConflictResolutionStrategy {
  ...
  mergeFunction?: (
    local: Partial<MatchUpdate>,
    remote: Partial<Match>
  ) => Partial<MatchUpdate>;
}
...
export function useConflictResolution({
   ...
   */
  const fetchCurrentMatch = useCallback(
...
export function useOptimisticMatchUpdate(
  defaultStrategy: ConflictResolutionStrategy = { name: "latest-wins" }
) {
  const { updateMatch, conflicts, resolveConflict } = useConflictResolution({
    defaultStrategy,
  });

  const updateMatchOptimistic = useCallback(
    async (
      matchId: string,
      updates: Partial<MatchUpdate>
    ): Promise<boolean> => {
