...
export function useRankings(playDateId: string): UseRankingsReturn {
  ...
  const [rankingChanges, setRankingChanges] = useState<Map<string, number>>(new Map())
   ...
   */
  const calculateRankingsFromData = useCallback(async (matchResults: MatchResult[]) => {
    try {
   ...
   */
  const handleRankingsUpdate = useCallback(async (matchResults: MatchResult[]) => {
    try {
   ...
   */
  const handleRankingsError = useCallback((err: Error) => {
    ...
    addToast({
      ...
      message: 'Lost connection to rankings updates. Some data may be outdated.',
      duration: 5000
    })
  }, [addToast])
...
export function useTournamentProgress(playDateId: string) {
  ...
  const loadProgress = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to load tournament progress')
    } finally {
      setLoading(false)
    }
  }, [playDateId])
