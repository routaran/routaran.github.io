...
export function usePlayerStats(
  ...
  const calculatePlayerStats = useCallback(async (
    ...
    const averageWinPercentage = winPercentages.length > 0 
      ...
      : 0
    const bestWinPercentage = winPercentages.length > 0 ? Math.max(...winPercentages) : 0
    const worstWinPercentage = winPercentages.length > 0 ? Math.min(...winPercentages) : 0
    ...
    if (playDateId) {
      try {
        ...
        const playerPartnerships = partnershipData.partnerships.filter(p => 
          p.player1_id === playerId || p.player2_id === playerId
        )
        
        partnershipStats = calculatePartnershipStats(playerPartnerships, partnershipData.matches)
        
        // Fill in player names
        partnershipStats.forEach(stats => {
          const partnership = partnershipData.partnerships.find(p => p.id === stats.partnership_id)
          if (partnership) {
            const player1 = partnershipData.players.find(p => p.id === partnership.player1_id)
            const player2 = partnershipData.players.find(p => p.id === partnership.player2_id)
            stats.player1_name = player1?.name || ''
            stats.player2_name = player2?.name || ''
          }
        })
        
        bestPartnership = partnershipStats.length > 0 
  ...
  const loadPlayerStats = useCallback(async () => {
    try {
      ...
      const { matchResults, playDates } = await getPlayerHistoricalStats(playerId)
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to refresh player statistics')
    } finally {
      setRefreshing(false)
    }
...
function calculateConsistencyRating(winPercentages: number[]): number {
  ...
  const consistencyScore = Math.max(0, Math.min(100, 100 - (standardDeviation / maxStdDev * 100)))
...
function calculateImprovementTrend(
  ...
  const recentAverage = recentGames.reduce((sum, stat) => 
    sum + stat.matchResult.win_percentage, 0
  ) / recentGames.length

  const earlierAverage = earlierGames.reduce((sum, stat) => 
    sum + stat.matchResult.win_percentage, 0
  ) / earlierGames.length
  ...
  const comparison = useMemo(() => {
    ...
    return {
      winPercentage: {
        ...
        winner: (p1.currentStats?.win_percentage || 0) > (p2.currentStats?.win_percentage || 0) ? 1 : 2
      },
      pointDifferential: {
        player1: p1.currentStats?.point_differential || 0,
        player2: p2.currentStats?.point_differential || 0,
        winner: (p1.currentStats?.point_differential || 0) > (p2.currentStats?.point_differential || 0) ? 1 : 2
      },
      consistency: {
