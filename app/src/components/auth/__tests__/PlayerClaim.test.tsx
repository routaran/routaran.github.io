...
const mockPlayers = [
  { id: '1', name: 'Alice Johnson', email: 'alice@example.com', claim_user_id: null },
  { id: '2', name: 'Bob Smith', email: 'bob@example.com', claim_user_id: null },
  { id: '3', name: 'Charlie Brown', email: 'charlie@example.com', claim_user_id: null },
];
...
describe('PlayerClaim', () => {
  ...
  it('renders player selection when players are available', async () => {
    ...
    expect(screen.getByRole('button', { name: /claim player/i })).toBeInTheDocument();
  ...
  it('auto-selects player when only one is available', async () => {
    ...
    await waitFor(() => {
      const selectElement = screen.getByLabelText(/your name/i) as HTMLSelectElement;
  ...
  it('shows empty state when no players are available', async () => {
    ...
    expect(screen.getByText(/all players have been claimed/i)).toBeInTheDocument();
    expect(screen.getByText(/contact your tournament organizer/i)).toBeInTheDocument();
  });
  ...
  it('has proper accessibility attributes', async () => {
    ...
    const selectElement = screen.getByLabelText(/your name/i);
    expect(selectElement).toHaveAttribute('aria-label', 'Select your player name');
  });
});