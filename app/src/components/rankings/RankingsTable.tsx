...
type SortField = 'rank' | 'name' | 'win_percentage' | 'point_differential' | 'games_played'
...
export function RankingsTable({
  ...
  const getRankingChangeIndicator = (playerId: string) => {
    ...
    return (
      <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${bgColorClass} ${colorClass}`}>
  ...
  const getRankDisplay = (rank: number) => {
    if (rank <= 3) {
      ...
      return (
        <div className={`inline-flex items-center px-2 py-1 rounded-full text-sm font-semibold ${bgColors[rank - 1]} ${colors[rank - 1]}`}>
            ...
            {sortedRankings.map((player) => (
              ...
              >
                ...
                <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-500">
                  ...
                  <span className="font-medium text-red-600">{player.games_lost}</span>
                </td>
                
                <td className="px-3 py-4 whitespace-nowrap">
                  <Badge 
                    variant={player.win_percentage >= 70 ? 'success' : 
                            player.win_percentage >= 50 ? 'warning' : 'error'}
                    className="text-xs"
                  >
                ...
                <td className="px-3 py-4 whitespace-nowrap text-sm">
                  <span className={`font-medium ${
                    player.point_differential > 0 ? 'text-green-600' : 
                    player.point_differential < 0 ? 'text-red-600' : 'text-gray-600'
                  }`}>
                ...
                {!compact && (
                  <td className="px-3 py-4 whitespace-nowrap text-sm text-gray-500">
                    <span className="font-medium text-green-600">{player.points_for}</span>
                    <span className="mx-1">-</span>
                    <span className="font-medium text-red-600">{player.points_against}</span>
                  </td>
                )}
 ...
 */
export function CompactRankingsTable(props: Omit<RankingsTableProps, 'compact'>) {
 ...
 */
export function RankingsPodium({ rankings, className = '' }: { rankings: PlayerRanking[]; className?: string }) {
  ...
  return (
    <Card className={`p-6 bg-gradient-to-br from-yellow-50 to-amber-50 ${className}`}>
      <h3 className="text-lg font-bold text-gray-900 mb-4 text-center">Tournament Podium</h3>
      ...
      <div className="flex items-end justify-center space-x-4">
        ...
        {topThree[1] && (
          <div className="text-center">
            <div className="bg-gray-100 rounded-lg p-4 mb-2">
              ...
              <p className="text-sm font-medium text-gray-900">{topThree[1].player_name}</p>
              <p className="text-xs text-gray-500">{topThree[1].win_percentage}% wins</p>
            </div>
        ...
        <div className="text-center">
          <div className="bg-yellow-100 rounded-lg p-4 mb-2">
            ...
            <p className="text-sm font-medium text-gray-900">{topThree[0].player_name}</p>
            <p className="text-xs text-gray-500">{topThree[0].win_percentage}% wins</p>
          </div>
        ...
        {topThree[2] && (
          <div className="text-center">
            <div className="bg-amber-100 rounded-lg p-4 mb-2">
              ...
              <p className="text-sm font-medium text-gray-900">{topThree[2].player_name}</p>
              <p className="text-xs text-gray-500">{topThree[2].win_percentage}% wins</p>
            </div>
