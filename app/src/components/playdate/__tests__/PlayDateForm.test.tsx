...
describe("PlayDateForm", () => {
  ...
  it("renders with initial data when editing", () => {
    ...
    expect(screen.getByLabelText("Play Date Name")).toHaveValue(
      "Test Play Date"
    );
  ...
  it("hides player selector when editing", () => {
    render(
      <PlayDateForm {...defaultProps} isEditing={true} canEditPlayers={false} />
    );
  ...
  it("validates required fields", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Play date name is required")
      ).toBeInTheDocument();
      expect(screen.getByText("Date is required")).toBeInTheDocument();
      expect(
        screen.getByText("At least 4 players are required")
      ).toBeInTheDocument();
    });
  ...
  it("validates target score range", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Target score must be between 5 and 21")
      ).toBeInTheDocument();
    });
  });
  ...
  it("submits form with valid data", async () => {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      const playerEmailInput =
        screen.getByPlaceholderText("player@example.com");
  ...
  it("shows update button text when editing", () => {
    render(<PlayDateForm {...defaultProps} isEditing={true} />);
  ...
  it("clears field errors when user types", async () => {
    ...
    expect(
      screen.queryByText("Play date name is required")
