...
describe("PlayDateCreatePage", () => {
  ...
  it("shows help text about what happens next", () => {
    ...
    expect(screen.getByText("What happens next?")).toBeInTheDocument();
    expect(
      screen.getByText(/A round-robin schedule will be automatically generated/)
    ).toBeInTheDocument();
    expect(
      screen.getByText(
    ...
    expect(
      screen.getByText(/Players can log in with their email to enter scores/)
    ).toBeInTheDocument();
    expect(
      screen.getByText(/Live rankings will update as matches are completed/)
    ).toBeInTheDocument();
  });
