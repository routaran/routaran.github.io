...
export function calculateRankings(
  ...
  const playDateResults = matchResults.filter(
  ...
  const headToHeadRecords = headToHeadData
    ? calculateHeadToHeadRecords(
        headToHeadData.matches,
        headToHeadData.partnerships
      )
  ...
  rankings.sort((a, b) => {
    ...
    if (a.head_to_head_record && b.head_to_head_record) {
      const aVsB = a.head_to_head_record.find(
        (record) => record.opponent_id === b.player_id
      );
      const bVsA = b.head_to_head_record.find(
        (record) => record.opponent_id === a.player_id
      );
...
export function calculateTournamentSummary(
  ...
  const averageMatchScore =
    allScores.length > 0 ? Math.round(totalPointsScored / allScores.length) : 0;
...
export function calculatePartnershipStats(
  ...
  return partnerships.map((partnership) => {
    const partnershipMatches = matches.filter(
      (match) =>
        (match.partnership1_id === partnership.id ||
          match.partnership2_id === partnership.id) &&
    ...
    const averagePointsPerGame =
      gamesPlayed > 0 ? Math.round(pointsFor / gamesPlayed) : 0;
...
export function calculateWinningStreak(
  ...
  const playerPartnerships = partnerships.filter(
  ...
  for (const match of playerMatches) {
    const playerPartnership = playerPartnerships.find(
      (p) => match.partnership1_id === p.id || match.partnership2_id === p.id
    )!;
