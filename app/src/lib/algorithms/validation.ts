...
function validateMatchConstraints(
  ...
  rounds.forEach(round => {
    ...
    round.matches.forEach(match => {
      ...
      const key = [match.partnership1.id, match.partnership2.id].sort().join('-');
      const reverseKey = [match.partnership2.id, match.partnership1.id].sort().join('-');
      
      if (matchups.has(key) || matchups.has(reverseKey)) {
        result.errors.push(`Duplicate matchup: ${match.partnership1.id} vs ${match.partnership2.id}`);
        result.isValid = false;
      }
      matchups.set(key, new Set([match.partnership1.id, match.partnership2.id]));
      ...
      players.forEach(playerId => {
        if (playersInRound.has(playerId)) {
          result.errors.push(`Player ${playerId} scheduled twice in round ${round.number}`);
          result.isValid = false;
        }
...
function validateRoundCompleteness(
  ...
  rounds.forEach(round => {
    round.matches.forEach(match => {
      matchCounts.set(match.partnership1.id, (matchCounts.get(match.partnership1.id) || 0) + 1);
      matchCounts.set(match.partnership2.id, (matchCounts.get(match.partnership2.id) || 0) + 1);
    });
  ...
  if (maxMatches - minMatches > 2) {
    result.warnings.push(`Uneven match distribution: ${minMatches}-${maxMatches} matches per partnership`);
  }
}
