...
export function generateRoundRobinSchedule(
  ...
  while (scheduledMatches.size < matches.length) {
    ...
    for (const match of matches) {
      ...
      const hasConflict = allPlayers.some((playerId) =>
        partnershipsInRound.has(playerId)
      );
...
export function validateSchedule(
  ...
  for (const [partnershipId, opponents] of matchupMap) {
    ...
    const expectedOpponents = partnerships.filter(
      (p) => p.id !== partnershipId && !hasSharedPlayer(partnership, p)
    );
...
export function getMatchesForPartnership(
  ...
  rounds.forEach((round) => {
    round.matches.forEach((match) => {
      if (
        match.partnership1.id === partnershipId ||
