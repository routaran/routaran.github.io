  ...
  describe("exportRankingsToCSV", () => {
    it("should generate valid CSV with correct headers", () => {
      ...
      expect(lines[0]).toContain(
        "Rank,Player Name,Games Played,Games Won,Games Lost"
      );
      expect(lines[0]).toContain(
        "Win Percentage,Points For,Points Against,Point Differential"
      );
  ...
  describe("exportPartnershipStatsToCSV", () => {
    it("should generate valid CSV for partnership statistics", () => {
      ...
      expect(lines[1]).toContain(
        '"Alice Johnson","Bob Smith",3,2,1,67,33,30,3,11'
      );
    });
  ...
  describe("generateExportFilename", () => {
    it("should generate correct filename for rankings", () => {
      ...
      expect(filename).toMatch(
        /^pickleball-rankings-2024-01-15-\d{4}-\d{2}-\d{2}\.csv$/
      );
    });
    ...
    it("should generate correct filename for player stats", () => {
      ...
      expect(filename).toMatch(
        /^pickleball-player-stats-2024-01-15-alice-johnson-\d{4}-\d{2}-\d{2}\.json$/
      );
    });
  ...
  describe("validateExportData", () => {
    ...
    it("should detect duplicate ranks", () => {
      ...
      expect(validation.errors).toContain(
        "Duplicate ranks found in rankings data"
      );
    });

    it("should detect negative values", () => {
      const negativeValues = [{ ...mockRankings[0], games_played: -1 }];
      const validation = validateExportData(negativeValues, mockPlayDate);

      expect(validation.valid).toBe(false);
      expect(validation.errors).toContain(
        "Invalid negative values found in rankings data"
      );
    });
  });
