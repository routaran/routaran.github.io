...
export async function getRankingsData(
  ...
  try {
    ...
    const [
      ...
      supabase.from("match_results").select("*").eq("play_date_id", playDateId),

      // Get all matches for head-to-head calculations
      supabase.from("matches").select("*").eq("play_date_id", playDateId),
    ...
    if (matchResultsResponse.error) {
      throw new Error(
        `Failed to fetch match results: ${matchResultsResponse.error.message}`
      );
    }
    if (matchesResponse.error) {
      throw new Error(
        `Failed to fetch matches: ${matchesResponse.error.message}`
      );
    }
    if (partnershipsResponse.error) {
      throw new Error(
        `Failed to fetch partnerships: ${partnershipsResponse.error.message}`
      );
    }
    if (playersResponse.error) {
      throw new Error(
        `Failed to fetch players: ${playersResponse.error.message}`
      );
    }
...
export async function getPlayerHistoricalStats(playerId: string): Promise<{
  ...
  try {
    ...
    if (matchResultsResponse.error) {
      throw new Error(
        `Failed to fetch player historical stats: ${matchResultsResponse.error.message}`
      );
    }

    // Get play date details
    const playDateIds = [
      ...new Set(matchResultsResponse.data?.map((r) => r.play_date_id) || []),
    ];
    ...
    if (playDateIds.length > 0) {
      ...
      if (playDatesResponse.error) {
        throw new Error(
          `Failed to fetch play dates: ${playDatesResponse.error.message}`
        );
      }
...
export async function getPartnershipStats(playDateId: string): Promise<{
  ...
  try {
    const [partnershipsResponse, matchesResponse, playersResponse] =
      await Promise.all([
        supabase
          .from("partnerships")
          .select("*")
      ...
      async (payload) => {
        try {
          ...
          if (response.error) {
            onError(
              new Error(
                `Failed to fetch updated match results: ${response.error.message}`
              )
            );
            return;
          }
...
export async function refreshMatchResultsView(): Promise<void> {
  try {
    const response = await supabase.rpc("refresh_match_results_view");

    if (response.error) {
      throw new Error(
        `Failed to refresh match results view: ${response.error.message}`
      );
    }
  } catch (error) {
 ...
 */
export async function getTournamentCompletionStats(
  playDateId: string
): Promise<{
  totalMatches: number;
  ...
  try {
    ...
    if (response.error) {
      throw new Error(
        `Failed to fetch tournament completion stats: ${response.error.message}`
      );
    }
...
> {
  try {
    ...
    if (response.error) {
      throw new Error(
        `Failed to fetch recent match results: ${response.error.message}`
      );
    }
...
> {
  try {
    ...
    if (response.error) {
      throw new Error(
        `Failed to fetch player performance trends: ${response.error.message}`
      );
    }
