...
describe('RealtimeManager', () => {
  ...
  describe('subscribe', () => {
    it('should create a subscription and return unsubscribe function', () => {
      ...
      expect(vi.mocked(supabase.channel)).toHaveBeenCalledWith(expect.stringMatching(/^realtime:matches/));
    ...
    it('should handle async callback errors', async () => {
      const callback = vi.fn().mockRejectedValue(new Error('Async callback error'));
      const onError = vi.fn();
      let capturedCallback: any;
