...
describe('Router', () => {
  ...
  it('has auth callback route', () => {
    const route = router.routes[0].children?.find(r => r.path === 'auth/callback');
    expect(route).toBeDefined();
  });

  it('has dashboard route', () => {
    const route = router.routes[0].children?.find(r => r.path === 'dashboard');
    expect(route).toBeDefined();
  });

  it('has protected routes', () => {
    const playDatesRoute = router.routes[0].children?.find(r => r.path === 'play-dates');
